...
# 熱門標籤：依標籤扣對應原料（牛奶不在這，改從商品配方算）
TAG_RULES = {
    # 配料（生重，每份）
    '珍珠':     [('珍珠(生)', 30)],   # ⚠️ 待優化：實際秤一平匙幾 g
    'QQ':       [('QQ(生)',   20)],   # ⚠️ 待優化：實際秤一匙幾 g
    # 茶凍（ml/杯，平均值，等 L/M 標籤明確再分）
    '桂花清凍': [('桂花清凍', 190)],
    '鐵觀音凍': [('鐵觀音凍', 190)],
    '綠茶凍':   [('綠茶凍',   190)],
}
...
with tab2:
    ...
    with st.expander("⚠️ 待優化細節（記錄中）"):
        ...
        - 🥛 **鮮奶**：從**配方表**精準算總 ml（L=230、M=180、歐蕾 60/50、可可 130/100），按**標籤品牌比例**分配給各品牌
        - 🍵 **茶凍 190ml/杯**：用平均，等 L/M 標籤明確再分
        - 🍯 **糖、雪霜、茶葉 g**：第一版**不扣**，第二版再做
        - 🛍️ **購物袋**：「購物袋L」→ 大袋；「購物袋」→ 小袋
        - 🥤 **客人單買珍珠/QQ**：目前**略過不扣**（列在「略過商品」），之後決定
    ...
    if sales_file or tag_file:
        ...
        recipes_idx = {}   # {product_name: [sizes]}
        recipes_full = {}  # {(name, size): recipe}
        for r in all_recipes:
            recipes_idx.setdefault(r['product_name'], []).append(r.get('size', ''))
            recipes_full[(r['product_name'], r.get('size', ''))] = r

        # 鮮奶相關統計
        total_milk_ml = 0      # 商品分析算出的總鮮奶 ml
        milk_drink_cups = 0    # 鮮奶飲料總杯數（從商品分析）
        milk_tag_count = {b: 0 for b in MILK_BOTTLES}  # 各品牌標籤杯數
        ...
        if sales_df is not None:
            for _, row in sales_df.iterrows():
                ...
                if size and size in PRODUCT_RULES:
                    ...
                    recipe = (recipes_full.get((norm_name, size))
                              or recipes_full.get((norm_name, '')))
                    if recipe:
                        milk_per = float(recipe.get('milk_ml') or 0)
                        if milk_per > 0:
                            total_milk_ml += milk_per * qty
                            milk_drink_cups += qty
                else:
        ...
        if tag_df is not None:
            for _, row in tag_df.iterrows():
                ...
                if qty <= 0:
                    continue
                # 牛奶標籤：累計到 milk_tag_count（後面按比例分攤）
                milk_matched = False
                for brand in MILK_BOTTLES:
                    if brand in tname:
                        milk_tag_count[brand] += qty
                        milk_matched = True
                        break
                if milk_matched:
                    continue
        ...
        # ── 鮮奶按品牌比例分攤 ──
        total_milk_tags = sum(milk_tag_count.values())
        milk_breakdown = []  # [(品牌, 杯數, 比例%, 分配ml, 瓶數)]
        if total_milk_ml > 0 and total_milk_tags > 0:
            for brand, count in milk_tag_count.items():
                if count == 0:
                    continue
                ratio = count / total_milk_tags
                ml_for_brand = total_milk_ml * ratio
                mat_name, ml_per_bottle = MILK_BOTTLES[brand]
                bottles = round(ml_for_brand / ml_per_bottle, 3)
                deductions[mat_name] = deductions.get(mat_name, 0) + bottles
                milk_breakdown.append({
                    '品牌': brand, '標籤杯數': count,
                    '比例': f"{ratio*100:.1f}%",
                    '分配 ml': f"{ml_for_brand:,.0f}",
                    '扣瓶數': bottles
                })
        ...
        c1.metric("鮮奶飲料", f"{milk_drink_cups} 杯")
        c2.metric("應扣鮮奶", f"{total_milk_ml:,.0f} ml")
        c3.metric("有打標籤", f"{total_milk_tags} 杯")

        gap = milk_drink_cups - total_milk_tags
        if gap > 0:
            st.warning(f"⚠️ 有 **{gap} 杯**鮮奶飲料**沒打牛奶品牌標籤** "
                       f"→ 這些 ml 也會按現有比例分配（因為不知道用哪牌）")
        elif gap < 0:
            st.info(f"ℹ️ 牛奶標籤比鮮奶飲料多 {-gap} 杯（有些可能是加奶但本身不算鮮奶飲料）")

        if milk_breakdown:
            st.dataframe(pd.DataFrame(milk_breakdown),
