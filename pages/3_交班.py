...
from db import (get_daily_report, upsert_daily_report, get_daily_reports_month,
                get_cash_expenses, add_cash_expense, upload_expense_photo,
                get_sales_records)
...
# 1️⃣ 匯入 CSV
# ═══════════════════════════════════════════════
with st.expander("1️⃣ 匯入肚肚 CSV（自動填收入）", expanded=not bool(existing)):
    st.caption("3 個 CSV 都選填，先傳了就會自動帶數字")
    c1, c2 = st.columns(2)
    pay_csv = c1.file_uploader("💳 付款方式彙總", type=['csv'], key="pay_csv")
    close_csv = c2.file_uploader("📊 關帳小結", type=['csv'], key="close_csv")
    cash_csv = st.file_uploader("💰 錢櫃管理（留存金）", type=['csv'], key="cash_csv")

    if 'auto_data' not in st.session_state:
        st.session_state.auto_data = {}

    if st.button("📥 解析 CSV", use_container_width=True):
        auto = {}
        # 付款方式彙總
        if pay_csv:
            try:
                df = pd.read_csv(pay_csv, encoding='utf-8-sig', header=4)
                # 找「金額」那行
                row = df[df.iloc[:, 0].astype(str).str.contains('金額|Total', case=False, na=False)]
                if len(row) > 0:
                    r = row.iloc[0]
                    def gv(col):
                        try:
                            return float(str(r.get(col, 0)).replace(',', '').strip() or 0)
                        except:
                            return 0
                    auto['cash_revenue'] = gv('現金')
                    auto['linepay_revenue'] = gv('LINE Pay')
                    auto['ubereats_revenue'] = gv('UberEats')
                    auto['foodpanda_revenue'] = gv('foodpanda')
                    auto['pos_total'] = gv('總計')
                st.success("✅ 付款方式彙總解析完成")
            except Exception as e:
                st.error(f"付款方式彙總解析失敗：{e}")

        # 關帳小結
        if close_csv:
            try:
                df = pd.read_csv(close_csv, encoding='utf-8-sig')
                # 篩選日期 + 「關帳」那行
                df['_date'] = df['營業日'].astype(str).str[:10]
                today_rows = df[df['_date'] == report_date_str]
                close_row = today_rows[today_rows['時間'].astype(str).str.contains('關帳', na=False)]
                if len(close_row) > 0:
                    r = close_row.iloc[0]
                    def gv2(col):
                        try:
                            ...
                            return 0
                    if 'pos_total' not in auto:
                        auto['pos_total'] = gv2('營業額')
                    if 'cash_revenue' not in auto:
                        auto['cash_revenue'] = gv2('現金')
                    auto['opening_cash'] = gv2('留存金')
                    auto['void_total'] = gv2('作廢總額')
                    auto['short_over'] = gv2('短溢')
                st.success(f"✅ 關帳小結解析完成（{report_date_str}）")
            except Exception as e:
                st.error(f"關帳小結解析失敗：{e}")

        # 錢櫃管理
        if cash_csv:
            try:
                df = pd.read_csv(cash_csv, encoding='utf-8-sig')
                df['_date'] = df['營業日'].astype(str).str[:10]
                rows = df[(df['_date'] == report_date_str) &
                          (df['類型'].astype(str).str.contains('留存金', na=False))]
                if len(rows) > 0:
                    auto['opening_cash'] = float(str(rows.iloc[0]['金額']).replace(',', ''))
                st.success("✅ 錢櫃管理解析完成")
            except Exception as e:
                st.error(f"錢櫃管理解析失敗：{e}")

        st.session_state.auto_data = auto
        if auto:
            st.info(f"已自動帶入 {len(auto)} 個欄位，請往下確認")
...
# 2️⃣ 收入
# ═══════════════════════════════════════════════
with st.expander("2️⃣ 💰 今日收入", expanded=True):
    c1, c2 = st.columns(2)
    cash_revenue = c1.number_input("💵 現金", value=_get_val('cash_revenue'),
                                    step=10.0, key="cash_revenue")
    linepay_revenue = c2.number_input("📱 LINE Pay", value=_get_val('linepay_revenue'),
                                       step=10.0, key="linepay_revenue")
    ubereats_revenue = c1.number_input("🛵 UberEats", value=_get_val('ubereats_revenue'),
                                        step=10.0, key="ubereats_revenue")
    foodpanda_revenue = c2.number_input("🛵 foodpanda", value=_get_val('foodpanda_revenue'),
                                         step=10.0, key="foodpanda_revenue")
    other_revenue = c1.number_input("➕ 其他收入", value=_get_val('other_revenue'),
                                     step=10.0, key="other_revenue")
    pos_total = c2.number_input("📊 POS 顯示總額", value=_get_val('pos_total'),
                                 step=10.0, key="pos_total")

    total_revenue = (cash_revenue + linepay_revenue + ubereats_revenue
                     + foodpanda_revenue + other_revenue)
    st.markdown(f"<div class='cash-card'><span class='sec-title'>合計：</span>"
                f"<span class='big-num'>${total_revenue:,.0f}</span></div>",
                unsafe_allow_html=True)
    if pos_total > 0 and abs(total_revenue - pos_total) > 1:
        st.warning(f"⚠️ 收入合計 ${total_revenue:,.0f} 與 POS ${pos_total:,.0f} 差 ${total_revenue-pos_total:+,.0f}")
...
with st.expander("6️⃣ ✅ 現金對帳", expanded=True):
    opening_cash = st.number_input(
        "期初現金（留存金）", value=_get_val('opening_cash'),
