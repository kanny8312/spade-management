...
# 5️⃣ 實際點錢
# ═══════════════════════════════════════════════
with st.expander("5️⃣ 🪙 實際點錢", expanded=True):
    actual_cash = st.number_input(
        "💵 實際盤點現金（一個數字就好）",
        min_value=0.0, step=100.0,
        value=float(_get_val('actual_cash', 0)),
        key="actual_cash_input",
        help="把錢盤所有現金加總後，輸入一個數字"
    )
    st.markdown(f"<div class='cash-card'>"
                f"<span class='sec-title'>實際盤點：</span>"
                f"<span class='big-num' style='color:#0d6efd'>${actual_cash:,.0f}</span>"
                f"</div>", unsafe_allow_html=True)
    # 保留欄位給匯出時用
    cash_thousand_count = 0
    cash_hundred_count = 0
...
if st.button("✅ 完成交班 / 儲存", use_container_width=True, type="primary"):
...
# ═══════════════════════════════════════════════
st.markdown("---")
st.subheader("📋 歷史紀錄（本月）")
hist_year = date.today().year
hist_month = date.today().month
hist_reports = get_daily_reports_month(hist_year, hist_month)
if hist_reports:
    rows = []
    for r in hist_reports:
        rows.append({
            '日期': r.get('report_date'),
            '狀態': '🔒 已交班' if r.get('status') == 'closed' else '🟡 草稿',
            '營業額': f"${float(r.get('pos_total') or 0):,.0f}",
            '現金': f"${float(r.get('cash_revenue') or 0):,.0f}",
            '應有': f"${float(r.get('expected_cash') or 0):,.0f}",
            '實際': f"${float(r.get('actual_cash') or 0):,.0f}",
            '差額': f"{float(r.get('cash_diff') or 0):+,.0f}",
            '交班人': r.get('handler', ''),
        })
    df_h = pd.DataFrame(rows)
    st.dataframe(df_h, use_container_width=True, hide_index=True)
    st.caption(f"共 {len(rows)} 筆記錄。要看其他日期，**回到頁面最上方改日期**即可載入")
else:
    st.caption("本月尚無記錄")
