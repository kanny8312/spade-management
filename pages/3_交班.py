...
with st.expander("6️⃣ ✅ 現金對帳", expanded=True):
    # 期初現金：優先用昨日結餘，沒有就手動輸入
    default_opening = _get_val('opening_cash', yesterday_cash or 0)
    if yesterday_cash is not None and not existing.get('opening_cash'):
        st.info(f"💡 已自動帶入昨日（{(datetime.fromisoformat(report_date_str)-timedelta(days=1)).date()}）"
